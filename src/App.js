  ...
  login: async () => {
    ...
    return {
      ...
      user: {
        id: '12345',
        name: 'Madesh Palani',
        email: 'Madesh.Palani@usbank.com.com',
        avatar: `https://ui-avatars.io/api/?name=Madesh+Palani&background=0078d4&color=fff`
      }
    };
  },
  logout: async () => {
...
const apiService = {
  sendMessage: async (message, token) => {
    ...
    const responses = [
      ...
      "I can help you with that. Here are a few options to consider..."
    ];
...
export default function App() {
  ...
  const handleLogin = async () => {
    setIsLoading(true);
    try {
      const authResult = await mockMicrosoftAuth.login();
  ...
  const handleLogout = async () => {
    try {
      await mockMicrosoftAuth.logout();
      setUser(null);
      setToken(null);
